...
public class MinimumIndexSumOfTwoLists {
    public String[] findRestaurant(String[] list1, String[] list2) {
        ...
        for(int i=0; i<list1.length; i++) {
            places.put(list1[i], i);
        }

        int lowestSum = Integer.MAX_VALUE;
        for(int i=0; i<list2.length; i++) {
            if(places.containsKey(list2[i])) {
                if(places.get(list2[i]) + i < lowestSum) {
                    favourites.clear();
                    favourites.add(list2[i]);
                    lowestSum = places.get(list2[i]) + i;
                } else if ((places.get(list2[i]) + i) == lowestSum) {
                    favourites.add(list2[i]);
                }
            }
        }

        String[] favArr = new String[favourites.size()];
        favArr = favourites.toArray(favArr);
