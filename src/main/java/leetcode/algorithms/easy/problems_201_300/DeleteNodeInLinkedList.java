...
public class DeleteNodeInLinkedList {
    public void deleteNode(ListNode node) {
        if(node.next == null) {
            node = null; //if tail node is to be removed, simply set it to null
        } else {
            node.val = node.next.val; //replace current node's value with next node's value
            node.next = node.next.next; //point to next next node
        }
    }
