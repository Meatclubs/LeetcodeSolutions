...
public class GroupOfSpecialEquivalentStrings {
    ...
    public int numSpecialEquivGroups(String[] A) {
        ...
        for(String str : A) {
            ...
            distinct.add(Arrays.toString(code));
        }

        return distinct.size();
    }

    public int numSpecialEquivGroupsV2(String[] A) {
        Set<String> distinct = new HashSet<>();
        int results = 0;

        for(String str : A) {
            if(distinct.add(getHashBySwap(str.toCharArray()))) {
                results++;
            }
        }

        /**
         * //using sets.size() takes more time, so instead better to just use an integer count
         */
        return results;
    }

    /**
     * Since word lengths are short, easier to just sort them
     */
