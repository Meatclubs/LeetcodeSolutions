 ...
 * size of the candy bar that Bob must exchange. If there are multiple answers, you may return any one of them.
...
public class FairCandySwap {
    public int[] fairCandySwap(int[] A, int[] B) {
        Set<Integer> aSizes = new HashSet<>();
        int aSum = 0;
        int bSum = 0;
        ...
        int halfDifference = (aSum - bSum) / 2;

        for(int candy : B) {
            if(aSizes.contains(candy+halfDifference)) {
                return new int[]{candy+halfDifference, candy};
            }
        }
